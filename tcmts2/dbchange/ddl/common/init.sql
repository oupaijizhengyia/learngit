...
CREATE TRIGGER `ins_medicine` AFTER INSERT ON `medicine`
FOR EACH ROW BEGIN
	insert into `medicine_relation`(hospital_medicine_name,initial_code,native_medicine_id,hospital_id)
	values(NEW.medicine_name,NEW.initial_code,NEW.id,0);
...
CREATE TABLE `recipe_file` (                                                                               
               ...
               `recipe_id` int(10) unsigned DEFAULT NULL COMMENT '处方id',                                            
               `create_time` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',                                                   
               `file_path` varchar(100) COLLATE utf8_unicode_ci DEFAULT NULL COMMENT '图片路径', 
	      	   `file_type` int(2) unsigned DEFAULT '1' COMMENT '1.配方图',                                      
               PRIMARY KEY (`id`),                                                                                      
               KEY `idx_recipe_id` (`recipe_id`)                                                                        
             ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci COMMENT='处方附件';
             
alter table recipe add column is_urgent tinyint(1) DEFAULT '0' COMMENT '是否加急';
alter table recipe add column union_id  int(10) unsigned DEFAULT '0' COMMENT '关联ID';

CREATE TABLE `warning` (                                                                               
               `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT 'id',                                           
               `recipe_id` int(10) unsigned DEFAULT NULL COMMENT '处方id',                                            
               `operate_type` int(3) unsigned DEFAULT NULL COMMENT '操作类型',                                                                                   
               `start_time` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '开始时间',
	       	   `end_time` timestamp NULL DEFAULT NULL COMMENT '结束时间',                                      
	       	   `warning_status` tinyint(1) DEFAULT '0' COMMENT '处理状态',	  
               PRIMARY KEY (`id`),                                                                                      
               KEY `idx_recipe_id` (`recipe_id`)                                                                        
             ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci COMMENT='预警表';
/**
